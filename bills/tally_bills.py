...
def fetch_bills(num_bills=5):
    l_map = get_ledger_map()
    
    # Fetch Purchase vouchers for April 2025 ONLY (matching invoice pattern)
    ...
    try:
        ...
        vouchers = soup.find_all('VOUCHER')[:num_bills]

        print(f"Showing first {num_bills} Purchase vouchers from April 2025...")
        print("=" * 160)
        ...
        for idx, v in enumerate(vouchers, 1):
            ...
            # Get Party A/C Name (Vendor) from PARTYNAME field
            ...
            print(f"{'='*160}")
            print(f"Date: {v_date}")
            print(f"Bill No: {v_no}")
            print(f"Party A/C Name (Vendor): {party_name}")
            print(f"Purchase Order No: {po_number}")
            print(f"Reference No (Vendor Invoice): {reference_number}")
            print(f"Vendor Address: {', '.join(vendor_address) if vendor_address else '(not provided)'}")
            # Display payment terms with source indicator
            if payment_terms:
                source_label = f" (from {terms_source})" if terms_source == "vendor" else ""
            ...
            # (excluding vendor, taxes, and rounding)
            if not purchase_ledger_from_item:
                ...
                for entry in v.find_all('LEDGERENTRIES.LIST') or v.find_all('ALLLEDGERENTRIES.LIST'):
                    ...
                    if name == party_name:  # Skip vendor
                        continue
            ...
            for entry in v.find_all('LEDGERENTRIES.LIST') or v.find_all('ALLLEDGERENTRIES.LIST'):
                ...
                # Check for ANY tax ledger (CGST, SGST, IGST, etc.) - look for "input" or "gst" in name
                name_lower = name.lower()
...
if __name__ == "__main__":
    print("=" * 80)
    print("TALLY BILL VIEWER")
    print("=" * 80)
    
    # Get user input for number of bills
    try:
        num = input("\nHow many bills do you want to fetch? (default: 5): ").strip()
        if num == "":
            num_bills = 5
        else:
            num_bills = int(num)
    ...
    print(f"\nFetching {num_bills} bill(s)...\n")
    fetch_bills(num_bills)
