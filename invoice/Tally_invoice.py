...
def fetch_invoices(num_invoices=5):
    l_map = get_ledger_map()
    
    # Fetch Tax Invoice vouchers for April 2025 ONLY
    ...
    try:
        ...
        vouchers = soup.find_all('VOUCHER')[:num_invoices]

        print(f"Showing first {num_invoices} Tax Invoice vouchers from April 2025...")
        print("=" * 160)
        ...
        for idx, v in enumerate(vouchers, 1):
            ...
            print(f"IRN: {irn[:50]}..." if len(irn) > 50 else f"IRN: {irn}")
            print(f"Ack. No: {irn_ack_no}")
            print(f"Ack Date: {irn_ack_date}")
...
if __name__ == "__main__":
    print("=" * 80)
    print("TALLY INVOICE VIEWER")
    print("=" * 80)
    
    # Get user input for number of invoices
    try:
        num = input("\nHow many invoices do you want to fetch? (default: 5): ").strip()
        if num == "":
            num_invoices = 5
        else:
            num_invoices = int(num)
            if num_invoices <= 0:
    ...
    print(f"\nFetching {num_invoices} invoice(s)...\n")
    fetch_invoices(num_invoices)
